...
echo generateItemName('weapon'). '<br>';
echo generateQuestName('weapon'). '<br>';
	...
	foreach($arrPattern as $strKey) {
		...
		if(count($arrKey) > 1) {
			...
			if(isset($arrWords[$strKey])) {
				// Word Key anhängen
				// todo schauen das alle was etc. gleich bleiben
				$strReturn .= $arrWords[$strKey] . ' ';
			} else {
				// Pattern matcht nicht neues pattern wählen
				return patternFiller($arrPatterns, $strType, $strSubType);
			}
		} else {
			// Fix Werte
			$strReturn .= $strKey . ' ';
